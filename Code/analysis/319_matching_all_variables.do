...
import delimited using "$data/constructed data/matched_panel_investment_LASSO.csv", clear
keep if matched_all_variables == 1
...
preserve 
	...
	* Merge with matched IDs, keeping only matches
	...
	graph export "$output/dcdh_binary_solar_investment_shift_matched_all_variables.pdf", replace
...
preserve
	...
	* Merge with matched IDs, keeping only matches
	...
	graph export "$output/dcdh_binary_wind_investment_shift_matched_all_variables.pdf", replace
...
do "$code/Code/analysis/319_2_matching_all_variables.do"

esttab dyn_* lab_* using "$output/reg_invest_lab_dem_matched_all_variables.tex", ///

...
import delimited using "$data/constructed data/investment_dataset_stata_excluding_crit_2.csv", ///
...
preserve 
	...
	eststo dyn_solar: did_multiplegt_dyn ///
		...
		graphoptions(legend(off) ///
                 ...
                 ylabel(-0.001(0.001)0.01, labsize(large)) ///
				 xlabel(, labsize(large)) ///
				 xsize(5.6) ///
				 pcycle(1) ///
                 name(graph_solar_binary, replace))	
	graph export "$output/dcdh_binary_solar_investment_shift_excluding_crit_2.pdf", replace
	...
	graph export "$output/dcdh_binary_wind_investment_shift_excluding_crit_2.pdf", replace
...
do "$code/Code/analysis/316_2_main_reg_results_excluding_crit_2.do"

esttab dyn_* lab_* using "$output/reg_invest_lab_dem_excluding_crit_2.tex", ///

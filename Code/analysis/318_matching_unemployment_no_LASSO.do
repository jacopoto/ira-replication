...
import delimited using "$data/constructed data/matched_panel_investment_unemployment_no_LASSO.csv", clear
keep if matched_unemployment == 1
* Keep only the ID variable (assuming it's called census_tract_id)
keep census_tract_id
* Remove duplicates if any
duplicates drop census_tract_id, force
* Save as temporary file
tempfile matched_ids
save `matched_ids'

* Import your full data
...
preserve 
	keep investment_*_binary census_tract_id period treatment_shift
	
	* Merge with matched IDs, keeping only matches
	merge m:1 census_tract_id using `matched_ids', keep(matched) nogen
	...
	graph export "$output/dcdh_binary_solar_investment_shift_matched_unemployment_no_LASSO.pdf", replace
...
preserve
	...
	* Merge with matched IDs, keeping only matches
	...
	graph export "$output/dcdh_binary_wind_investment_shift_matched_unemployment_no_LASSO.pdf", replace
...
display "Solar Investment: DiD percent change relative counterfactual mean = " percent_change_solar_invest "%"

* Run labor demand do to eststore estimates to be shown in table
do "$code/Code/analysis/318_2_matching_unemployment_no_LASSO.do"

esttab dyn_* lab_* using "$output/reg_invest_lab_dem_matched_unemployment_no_LASSO.tex", ///

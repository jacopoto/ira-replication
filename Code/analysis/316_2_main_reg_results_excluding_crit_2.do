...
import delimited using ///
	"$data/constructed data/final_labor_demand_dataset_stata_excluding_crit_2.csv", ///
...
preserve 
	...
	eststo lab_solar: did_multiplegt_dyn ///
		...
		graphoptions(legend(off) ///
				...
				ylabel(-0.002(0.001)0.007, labsize(large)) ///
				xlabel(, labsize(large)) ///
				xsize(5.6) ///
				pcycle(1) ///
				name(graph_solar_labor_demand, replace))
	graph export "$output/dcdh_solar_labor_demand_shift_excluding_crit_2.pdf", replace
	...
	graph export "$output/dcdh_wind_labor_demand_shift_excluding_crit_2.pdf", replace

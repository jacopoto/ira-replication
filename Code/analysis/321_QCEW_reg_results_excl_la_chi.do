...
did_multiplegt_dyn all_employment county_id period treatment_shift, ///
    ...
    graphoptions(legend(off) ///
				 ...
				 ylabel(-6500(1500)4000, labsize(large)) ///
				 xlabel(, labsize(large)) ///
				 xsize(5.6) ///
				 pcycle(1) ///
                 plotregion(margin(zero)) ///
                 name(graph_all_employment, replace))	
graph export "$output/dcdh_all_employment_excl_la_chi.pdf", replace
...
graph export "$output/dcdh_construction_employment_excl_la_chi.pdf", replace

est sto model_constr

esttab model_* using "$output/rob_empl_excl_la_chi.tex", ///
	replace booktabs se nonumber nonote wide noobs ///
	mtitles("Total Employment" "Employment in Construction") ///
	b(%9.3f) se(%9.3f) ///
